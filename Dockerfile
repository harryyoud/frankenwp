...
WORKDIR /var/www/wp_root
ARG USER

RUN mkdir -p /var/www/wp_root \
    && mkdir /var/www/wp_cache \
    && mkdir /var/www/static \
    && chown -R ${USER}:${USER} /var/www/wp_root \
    && chown -R ${USER}:${USER} /var/www/wp_cache \
    && chown -R ${USER}:${USER} /var/www/static
VOLUME /var/www/wp_root/wp-content
VOLUME /var/www/wp_root/static

COPY wp-content/mu-plugins /var/www/wp_muplugins
...
RUN sed -i 's/<?php/<?php if (!!getenv("FORCE_HTTPS")) { \$_SERVER["HTTPS"] = "on"; } define( "FS_METHOD", "direct" ); set_time_limit(300); define( "WPMU_PLUGIN_DIR", "\/var\/www\/wp_muplugins\/" ); /g' /usr/src/wordpress/wp-config-docker.php
...
RUN chown -R ${USER}:${USER} /data/caddy && \
    ...
    chown -R ${USER}:${USER} /var/www/wp_root && \
    chown -R ${USER}:${USER} /var/www/wp_muplugins && \
    chown -R ${USER}:${USER} /var/www/wp_cache && \
    chown -R ${USER}:${USER} /var/www/static && \
